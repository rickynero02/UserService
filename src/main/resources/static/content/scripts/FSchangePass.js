...
function recoverPasswd(){
    ...
    if(uncheckedEmail.value === ""){
      ...
      sendRequest("GET","http://localhost:8080/changePassword?email="+uncheckedEmail.value,requirePassRecover)
    }
}
...
function changePass(){
  ...
  if(newPass.value === confNewPass.value){
    ...
    sendRequest("POST","http://localhost:8080/sendNewPassword",verifyPasswdModification,stringToSend)
  }
  else
  {
